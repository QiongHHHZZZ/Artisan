        ...
        public override void Draw()
        {
            try
            {
                ...
                if (autoMode && !P.Config.ReplicateMacroDelay)
                {
                    ...
                    ImGui.SliderInt(L10n.Tr("Set delay (ms)"), ref _delay, 0, 1000);
                ...
                if (!P.Config.AutoMode)
                {
                    ...
                    if (P.Config.UseDoNextX)
                    {
                        if (_doNextCounter <= 0)
                        {
                            if (ImGui.Button(L10n.Tr("Do Next {0} Actions", P.Config.DoNextXAmount)))
                            {
                        ...
                        {
                            if (ImGui.Button(L10n.Tr("Cancel Queue")))
                            {
                                _doNextCounter = 0;
                                _doNextTotal = 0;
                            }
                        ...
                        if (_doNextCounter > 0)
                        {
                            ...
                            ImGuiEx.Text(L10n.Tr("Processing action {0}/{1}", remaining, _doNextTotal));
                        }
                    }
                    else
                    {
