    ...
    public static class SimulatorUI
    {
            ...
            get
            {
                ...
                unsafe
                {
                    fixed (GearsetEntry?* gs = &SimGS)
                    {
                        ...
                        return $"{name} (ilvl {val.ItemLevel}){(materiaDiff ? T(" 警告：检测到魔晶石配置不同，请更新配装。") : "")}";
                    }
                }
            }
        }
        ...
        private static unsafe void DrawGearSetDropdown()
        {
            ...
            if (!CustomStatMode)
            {
                ...
                if (validGS == 0)
                {
                    ImGuiEx.Text(T("请添加一个 {0} 的配装。", LuminaSheets.ClassJobSheet[SelectedRecipe!.Value.CraftType.RowId + 8].Abbreviation));
                    SimGS = null;
                    return;
                }
                if (validGS == 1)
                {
                    ...
                    ImGui.SameLine(120f);
                    ImGuiEx.SetNextItemFullWidth();
                    ImGuiEx.Text($"{name} (ilvl {SimGS?.ItemLevel}){(materiaDiff ? T(" 警告：检测到魔晶石配置不同，请更新配装。") : "")}");
                    return;
                }
                ...
                foreach (var gs in Instance()->Entries)
                {
                    ...
                    string name = gs.NameString;
                    bool materiaDiff = gs.Items.ToArray().Any(x => x.Flags.HasFlag(GearsetItemFlag.MateriaDiffers));
                    var selected = ImGui.Selectable($"{name} (ilvl {gs.ItemLevel}){(materiaDiff ? T(" 警告：检测到魔晶石配置不同，请更新配装。") : "")}##GS{gs.Id}");
        ...
        private static void DrawRecipeInfo()
        {
            if (ingredientLayouts.TryGetValue(SelectedRecipe!.Value.RowId, out var layouts))
            {
                ...
                {
                    ImGuiEx.Text(T(SelectedRecipe!.Value.IsExpert ? "专家配方" : SelectedRecipe!.Value.SecretRecipeBook.RowId > 0 ? "秘传配方" : "普通配方"));
                });
        ...
        private static void DrawIngredientLayout()
        {
            ...
            ImGuiEx.LineCentered("###LayoutIngredients", () => ImGuiEx.TextUnderlined(T("材料配置")));
            ImGui.Spacing();
            ...
            ImGui.TableSetupColumn(T("材料"), ImGuiTableColumnFlags.WidthFixed, materialColumnWidth);
