    ...
    public static bool CannotUseAction(CraftState craft, StepState step, Skills action, out string reason)
    {
        if (!CanUseAction(craft, step, action))
        {
            reason = action switch
            {
                ...
                Skills.CarefulObservation => craft.Specialist ? Crafting.DelineationCount() == 0 ? UI.L10n.Tr("你的能工巧匠图纸已用完。") : UI.L10n.Tr("你已经使用过 {0} {1} 次。", Skills.CarefulObservation.NameOfAction(), 3) : UI.L10n.Tr("你不是专家。"),
                Skills.HeartAndSoul => craft.Specialist ? Crafting.DelineationCount() == 0 ? UI.L10n.Tr("你的能工巧匠图纸已用完。") : UI.L10n.Tr("{0} 已不再可用于本次制作。", Skills.HeartAndSoul.NameOfAction()) : UI.L10n.Tr("你不是专家。"),
