    ...
    public bool Draw()
    {
        ...
        try
        {
            ...
            if (ImGui.CollapsingHeader($"主循环 - {QualityString}前阶段"))
            {
                ...
                ImGuiComponents.HelpMarker($"启用后会尽快使用 {Buffs.Veneration.NameOfBuff()} 与 {Skills.RapidSynthesis.NameOfAction()} 冲满进度，不再优先考虑 {Buffs.InnerQuiet.NameOfBuff()} 层数或当前 {ConditionString.ToLower()}（更稳完工但灵活性较低）。\n禁用后会先叠满 {Buffs.InnerQuiet.NameOfBuff()} 再强推{ProgressString.ToLower()}（更灵活，但极端情况可能来不及完工）。");
            ...
            if (ImGui.CollapsingHeader($"主循环 - {QualityString}阶段"))
            {
                ...
                ImGuiComponents.HelpMarker($"尽管品质提升很大，但通常效率仍低于 {Skills.PreciseTouch.NameOfAction()}。");
                ...
                ImGui.TextWrapped($"当 ● {Condition.Good.ToLocalizedString()} 且仅有 {Skills.GreatStrides.NameOfAction()} 时：");
                ImGuiComponents.HelpMarker($"这里的“免费 {Skills.PreparatoryTouch.NameOfAction()}”指通过 {Skills.TrainedPerfection.NameOfAction()} 触发（可在前一阶段设置）。\n把 {Skills.QuickInnovation.NameOfAction()} 留给收尾阶段的紧急 {Skills.ByregotsBlessing.NameOfAction()} 通常效率更高，但并非总是必须。");
            ...
            if (ImGui.CollapsingHeader($"收尾阶段"))
            {
                ...
                ImGuiComponents.HelpMarker($"当 {Skills.GreatStrides.NameOfAction()} + {Skills.ByregotsBlessing.NameOfAction()} 足以收尾，但 CP 不足以使用 {Skills.GreatStrides.NameOfAction()} 或常规 {Skills.Observe.NameOfAction()} 时触发。");
                ...
                ImGui.Unindent();
                changed |= ImGui.Checkbox($"当CP不足时允许用 {Skills.QuickInnovation.NameOfAction()} 收尾", ref FinisherUseQuickInno);
                ImGuiComponents.HelpMarker($"当 CP 不足以使用 {Skills.Innovation.NameOfAction()} 和/或 {Skills.GreatStrides.NameOfAction()}，但 {Skills.QuickInnovation.NameOfAction()} 足以达到目标{QualityString.ToLower()}时触发。");
                changed |= ImGui.Checkbox($"在无其它选择时允许用 {Skills.RapidSynthesis.NameOfAction()} 收尾", ref RapidSynthYoloAllowed);
                ImGuiComponents.HelpMarker($"若禁用，求解器会停手不动，可能中断AFK专家制作。通常可安全启用，因为仅在CP或{DurabilityString.ToLower()}都见底时才会触发。");
                ImGui.Dummy(new Vector2(0, 5f));
            }
            ImGui.Unindent();
            ...
            changed |= ImGui.SliderInt($"每次制作最多使用 {Skills.SteadyHand.NameOfAction()} 次数###MaxSteadyUses", ref MaxSteadyUses, 0, 2);
            ImGuiComponents.HelpMarker($"{Skills.SteadyHand.NameOfAction()} 会尽早使用以保证 {Skills.RapidSynthesis.NameOfAction()} 成功。设为 0 可禁用。");
            ImGui.Unindent();
