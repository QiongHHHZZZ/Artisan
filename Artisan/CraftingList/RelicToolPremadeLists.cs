...
internal static partial class RelicToolPremadeLists
{
    ...
    public static void EnsureBuilt(List<NewCraftingList> premadeCraftingLists)
    {
        ...
        foreach (RelicToolPremadeEntry def in Definitions)
        {
            ...
            NewCraftingList? existingList = premadeCraftingLists.FirstOrDefault(x => x.ID == id);
            if (existingList != null)
            {
                if (existingList.IsPremade)
                {
                    existingList.Name = ListName(def.Step, subStep);
                }
    ...
    private static string ListName(RelicToolStep step, uint craftType)
    {
        string jobName = CultureInfo.InvariantCulture.TextInfo.ToTitleCase(LuminaSheets.ClassJobSheet[craftType + 8].Name.ToString());
        return L10n.TranslateRelicToolListName($"Relic Tool — {StepLabel(step)} — {jobName}");
    }
    ...
    private static bool TryBuildList(RelicToolPremadeEntry def, uint id, out NewCraftingList? list)
    {
        ...
        list = new NewCraftingList
        {
            ...
            Name = ListName(def.Step, recipe.CraftType.RowId),
            IsPremade = true,
        };
        list.Locked = true;
