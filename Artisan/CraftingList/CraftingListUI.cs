...
namespace Artisan.CraftingLists
{
    internal class CraftingListUI
    {
        ...
        private static void DrawListOptions()
        {
            ...
            {
                if (ImGui.BeginTabItem(L10n.Tr("User Lists")))
                {
                    UserListsUI.DrawLocalized(ImGui.GetContentRegionAvail().X);
                    ImGui.EndTabItem();
                }
                if (ImGui.BeginTabItem(L10n.Tr("Premade Lists")))
                {
                    try
                    {
                        P.PremadeLists.PremadesUI.DrawLocalized(ImGui.GetContentRegionAvail().X);
                    }
                    catch { }
                    ImGui.EndTabItem();
                }
